...
- Twin Card print layout is deterministic in [print-composer.mjs](/C:/w/myveevee/aws/twin-card/print-composer.mjs:1): marketing-sample white frame, top SWCA summit line, avatar image well with first-name overlay, right-side goal/next-steps copy, hardcoded `Visit myveevee.com to learn more` CTA, provided VeeVee SVG logo assets, and the provided SWCA vector logo asset in the footer.
...
- `/twin-dashboard` has separate tabs for Runs, Images, and Cards. Images is for raw/generated model QA. Cards is for final Canon-ready PNGs, booth printing, generated-card tally, printed-card tally, and per-row `printedCount` tracking through the PIN-gated admin API.
