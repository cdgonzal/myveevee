...
The revised simulation composition replaces the miniature Theo with a newly generated Vitruvian Nia, preserving her red hair, blue digital appearance and complete figure. It sits higher on the left inside a midnight-filled, cyan-outlined SVG thought cloud, with shrinking dots leading toward seated Nia's head. Source and exact built-in image_gen prompt are preserved in `codex/creative/nia-simulation/`; `node scripts/build-nia-simulation-asset.mjs` creates the lossless master, responsive delivery assets and provenance manifest. The older Theo asset remains available as a historical source. Nia stays on the right, fully visible. This illustrates simulation exploration rather than a real patient scan or product interface. The combined image description includes both subjects; the decorative thought layer has empty alt text to avoid a duplicate announcement.
